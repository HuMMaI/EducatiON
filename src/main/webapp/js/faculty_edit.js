...
$.get("/faculty/api/edit?id=" + id)
    .done(function (faculty) {
        $("#faculty-name").val(faculty.name);
        $("#number-of-seats").val(faculty.numberOfSeats);
            ...
            .done(function (data) {
                ...
                jQuery.each(data, function (key, item) {
                    subjects += "<div class=\"input-group mb-3\">\n" +
                        ...
                        "<input type=\"checkbox\" class=\"subject\" aria-label=\"Checkbox for following text input\" name=\"" + key + "\" " + (faculty.subjects.includes(key) ? 'checked' : '') + " >\n" +
                        "</div>\n" +
                        "</div>\n" +
